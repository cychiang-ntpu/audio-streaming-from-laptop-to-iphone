...
### 🎯 這個專案做什麼? / What does this project do?

使用 **Raspberry Pi Zero 2 W** + **2顆 I2S MEMS 麥克風** 打造一個 **高音質立體聲錄音與即時串流系統**。

Build a **high-quality stereo recording and live streaming system** using **Raspberry Pi Zero 2 W** + **2× I2S MEMS microphones**.

### 🔧 用什麼材料? / What hardware do you need?

- **主控板 / Main Board**: Raspberry Pi Zero 2 W (或 Pi 3/4)
- **麥克風 / Microphones**: 2× DFRobot SEN0526 (I2S MEMS 數位麥克風)
- **連接 / Wiring**: 杜邦線或麵包板 (I2S: GPIO 18/19/20)
- **電源 / Power**: 5V 2.5A+ 電源供應器 (或選配 UPS 模組)

### ✨ 可以做什麼? / What can it do?

1. **🎙️ 立體聲錄音 / Stereo Recording**
   - 48kHz / 16-bit 高音質錄音
   - 左右聲道獨立 (L/R channels)
   - ALSA 系統整合，支援各種錄音軟體

2. **📡 即時串流到手機 / Live Streaming to Mobile**
   - MediaMTX RTSP 伺服器 (超低延遲)
   - AAC 320kbps 高音質編碼
   - 透過 iPhone 熱點無線串流
   - VLC 播放器即時收聽 (延遲 < 100ms)

3. **⚙️ 自動化運行 / Automated Operation**
   - systemd 開機自動啟動錄音/串流
   - 背景運行，無需手動操作
   - 適合長時間監控或表演使用
...
### 📷 系統架構示意圖 / System Architecture Diagram

```
┌─────────────────────────────────────────────────────────────────────┐
│                     完整系統架構 / Complete System                     │
└─────────────────────────────────────────────────────────────────────┘

┌──────────────────────┐         ┌──────────────────────┐
│  🎙️ Left Mic (L)     │         │  🎙️ Right Mic (R)    │
│  DFRobot SEN0526     │         │  DFRobot SEN0526     │
│  ┌────────────────┐  │         │  ┌────────────────┐  │
│  │ SEL: GND (L)   │  │         │  │ SEL: 3.3V (R)  │  │
│  │ I2S Output     │  │         │  │ I2S Output     │  │
│  └────────────────┘  │         │  └────────────────┘  │
└──────────┬───────────┘         └───────────┬──────────┘
           │                                 │
           │ DO (Tri-state)                  │ DO (Tri-state)
           └────────┬────────────────────────┘
                    │ (Shared I2S Bus)
                    ▼
        ┌───────────────────────────────┐
        ...
        │  ┌─────────────────────────┐  │
        │  │ GPIO 18 (SCK/BCLK)  ◄───┼──┼─── Clock
        │  │ GPIO 19 (WS/LRCLK)  ◄───┼──┼─── L/R Select
        │  │ GPIO 20 (SD/DO)     ◄───┼──┼─── Data (Stereo)
        │  └─────────────────────────┘  │
        │                                │
        │  🔧 Software Stack:            │
        │  • ALSA (googlevoicehat)       │
...
麥克風聲道選擇 / Mic Channel Selection:
• Left Mic:  SEL → GND (輸出在 WS=LOW 時)
• Right Mic: SEL → 3.3V (輸出在 WS=HIGH 時)
```

**📸 實際硬體圖片 / Actual Hardware Photos**

> 💡 **提示 / Tip**: 如果您有實際製作成品的照片，可以在此處加入：
> 
> ```markdown
> ![System Overview](images/system-overview.jpg)
> ![Wiring Detail](images/wiring-detail.jpg)
> ![Running System](images/running-system.jpg)
> ```
> 
> 建議拍攝角度 / Recommended Photo Angles:
> - 整體系統俯視圖 (Overall system top view)
> - GPIO 接線特寫 (GPIO wiring close-up)
> - 兩個麥克風位置 (Two microphones positioning)
> - iPhone VLC 播放畫面 (iPhone VLC player screen)

---

> **Quick Guide / 快速說明**: 完整教學說明如何將 **兩顆** DFRobot **SEN0526** I2S MEMS 麥克風接到 **Raspberry Pi Zero 2 W** 並使用 ALSA 錄製 **立體聲（L/R）** 音訊，並透過 MediaMTX 即時串流到 iPhone。
>
> This guide shows how to wire **two** DFRobot **SEN0526** I2S MEMS microphones to a **Raspberry Pi Zero 2 W**, record **stereo (L/R)** audio using ALSA, and stream live to iPhone via MediaMTX.
